...
def inspect_row(i,M,C,row,col,jj=None):

    #scroll row
    for j in range(row[i],len(M)):
        if j!=jj:
            #print (i,j)
            #print " ",M[i][j]
            if M[i][j]==1:
...
if __name__ == "__main__":
    ...
    try:
        ...
        M=[[0 for i in range(len(monitored_graph.nodes()))] for k in range(len(monitored_graph.nodes()))]
        ...
        clusters=clustering(M, len(monitored_graph.nodes()),len(incremental_edges))
