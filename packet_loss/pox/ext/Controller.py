        ...
        if self.connection.dpid <= 40:
            ...
            if flag_match_S == 1:
                ...
                if var1 == 1 and var2 == 1 and var3 == 1 and var4 == 1:
                    ...
                    port = 52
                    tos = 0x04
                    self.matching_flag_rule_in_switchS_macs_macd_ips_ipd(
                        port, tos)
...
#match the flag in the packet and set the number of port of switch in the TOS field.....monitoring with filter on macs and macd
    ...
    def func1_macs_macd(self, mac_address_D, ip_address_D, macs, macd, tos,
                        nwtos):
...
#match the flag in the packet and set the number of port of switch in the TOS field.....monitoring with filter on ips and macs
    ...
    def func1_ips_macs(self, mac_address_D, ip_address_D, ips, macs, tos,
                       nwtos):
...
#match the flag in the packet and set the number of port of switch in the TOS field.....monitoring with filter on ipd and macd
    ...
    def func1_ipd_macd(self, mac_address_D, ip_address_D, ipd, macd, tos,
                       nwtos):
...
#match the flag in the packet and set the number of port of switch in the TOS field.....monitoring with filter on ips and macs and ipd and macd
    ...
    def func1_ips_ipd_macs_macd(self, mac_address_D, ip_address_D, ips, ipd,
                                macs, macd, tos, nwtos):
        ...
        if self.connection.dpid <= 40:
            ...
            if flag_forward_hp == 1:
                ...
                if var1 == 1 and var2 == 1 and var3 == 1 and var4 == 1:
                    ...
                    port = ((16 << 3) + 4)
                    tos = ((0 << 3) + 4)
                    self.ForwardingRule_SwitchS_macs_macd_ips_ipd(
                        port, tos, outport)
                    ...
                    for porta in dict2[dpid].keys():
                        ...
                        port = ((porta << 3) + 4)
                        tos = ((0 << 3) + 4)
                        self.ForwardingRule_SwitchS_macs_macd(
                            port, tos, outport)
                    ...
                    for porta in dict2[dpid].keys():
                        ...
                        port = ((porta << 3) + 4)
                        tos = ((0 << 3) + 4)
                        self.ForwardingRule_SwitchS_macs_ips(
                            port, tos, outport)
                    ...
                    for porta in dict2[dpid].keys():
                        ...
                        port = ((porta << 3) + 4)
                        tos = ((0 << 3) + 4)
                        self.ForwardingRule_SwitchS_macd_ipd(
                            port, tos, outport)
        ...
        if self.connection.dpid <= 40:
            ...
            if flag_set_port == 1:
                ...
                if var1 == 1 and var2 == 1 and var3 == 1 and var4 == 1:
                    ...
                    tos = 0x04
                    nwtos = ((16 << 3) + 4)
                    self.func1_ips_ipd_macs_macd(mac_d, ip_d, ip_src, ip_dst,
                                                 mac_src, mac_dst, tos, nwtos)
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            mac_destination = "00:00:00:00:00:" + str(
                                s[len(s) - 1] + 10)

                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1_ips_ipd_macs_macd(
                                mac_destination, ip_destination, ip_src,
                                ip_dst, mac_src, mac_dst, tos, nwtos)
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1_ips_ipd(mac_destination, ip_destination,
                                               ip_src, ip_dst, tos, nwtos)
                    ...
                    tos = 0x04
                    nwtos = ((16 << 3) + 4)
                    self.func1_macs_macd(mac_d, ip_d, mac_src, mac_dst, tos,
                                         nwtos)
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1_macs_macd(mac_destination,
                                                 ip_destination, mac_src,
                                                 mac_dst, tos, nwtos)
                    ...
                    tos = 0x04
                    nwtos = ((16 << 3) + 4)
                    self.func1_ips_macs(mac_d, ip_d, ip_src, mac_src, tos,
                                        nwtos)
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1_ips_macs(mac_destination,
                                                ip_destination, ip_src,
                                                mac_src, tos, nwtos)
                    ...
                    tos = 0x04
                    nwtos = ((16 << 3) + 4)
                    self.func1_ipd_macd(mac_d, ip_d, ip_dst, mac_dst, tos,
                                        nwtos)
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1_ipd_macd(mac_destination,
                    ...
                    for s in ShortPath[0]:
                        ...
                        if len(s) > 1:
                            ...
                            tos = 0x04
                            nwtos = ((s1_out << 3) + 4)
                            self.func1(mac_destination, ip_destination, tos,
                                       nwtos)
                ...
                if flag_forward_lp2 == 1:
                    ...
                    for s in spath_dpid:
                        ...
                        if len(s) > 1:
                            ...
                            msg.actions.append(
                                of.ofp_action_output(port=s1_out))
                            self.connection.send(msg)
    ...
    if event.connection.dpid <= 40 and event.connection.dpid != old:
            ...
            if flag_flow_stats == 1:
                ...
                for index, elem in enumerate(c):
                    if elem != 0:
                        ...
                        item['Direction'] = direct1
                        finaloutput0_0[porta1] = c[porta1] - Count_out0_PORTS[(
                            dpid * 41) + porta1]
                ...
                for index, element in enumerate(d):
                    if element != 0:
                        ...
                        item['Direction'] = direct2
                        finalinput0_0[porta] = d[porta] - Count_in0_PORTS[(
                            dpid * 41) + porta]
                ...
                for index, elem in enumerate(c):
                    if elem != 0:
                        ...
                        item['Direction'] = direct1
                        finaloutput0_1[porta2] = c[porta2] - Count_out1_PORTS[(
                            dpid * 41) + porta2]
                ...
                for index, element in enumerate(d):
                    if element != 0:
                        ...
                        item['Direction'] = direct2
                        finalinput0_1[porta3] = d[porta3] - Count_in1_PORTS[(
                            dpid * 41) + porta3]
        ...
        if C_flow_stats == N:
            ...
            for pid in range(0, 41):
                print "DPID : ", pid, "Count input 1 : ", Count_in1_PORTS[(
                    pid * 41
                ) + 1], "", Count_in1_PORTS[
                    (pid * 41) +
                    2], "", Count_in1_PORTS[(pid * 41) + 3], Count_in1_PORTS[
                        (pid * 41) +
                        4], Count_in1_PORTS[(pid * 41) + 5], Count_in1_PORTS[
                            (pid * 41) + 6], Count_in1_PORTS[
                                (pid * 41) + 7], Count_in1_PORTS[
                                    (pid * 41) + 8], Count_in1_PORTS[
                                        (pid * 41) +
                                        9], Count_in1_PORTS[(pid * 41) + 10]

            for pid in range(0, 41):
                print "DPID : ", pid, "Count output 1: ", Count_out1_PORTS[(
                    pid * 41
                ) + 1], "", Count_out1_PORTS[
                    (pid * 41) +
                    2], "", Count_out1_PORTS[(pid * 41) + 3], Count_out1_PORTS[
                        (pid * 41) +
                        4], Count_out1_PORTS[(pid * 41) + 5], Count_out1_PORTS[
                            (pid * 41) + 6], Count_out1_PORTS[
                                (pid * 41) + 7], Count_out1_PORTS[
                                    (pid * 41) + 8], Count_out1_PORTS[
                                        (pid * 41) +
                                        9], Count_out1_PORTS[(pid * 41) + 10]

            for pid in range(0, 41):
                print "DPID : ", pid, "Count input 0: ", Count_in0_PORTS[(
                    pid * 41
                ) + 1], "", Count_in0_PORTS[
                    (pid * 41) +
                    2], "", Count_in0_PORTS[(pid * 41) + 3], Count_in0_PORTS[
                        (pid * 41) +
                        4], Count_in0_PORTS[(pid * 41) + 5], Count_in0_PORTS[
                            (pid * 41) + 6], Count_in0_PORTS[
                                (pid * 41) + 7], Count_in0_PORTS[
                                    (pid * 41) + 8], Count_in0_PORTS[
                                        (pid * 41) +
                                        9], Count_in0_PORTS[(pid * 41) + 10]

            for pid in range(0, 41):
                print "DPID : ", pid, "Count output 0: ", Count_out0_PORTS[(
                    pid * 41
                ) + 1], "", Count_out0_PORTS[
                    (pid * 41) +
                    2], "", Count_out0_PORTS[(pid * 41) + 3], Count_out0_PORTS[
                        (pid * 41) +
                        4], Count_out0_PORTS[(pid * 41) + 5], Count_out0_PORTS[
                            (pid * 41) + 6], Count_out0_PORTS[
                                (pid * 41) + 7], Count_out0_PORTS[
                                    (pid * 41) + 8], Count_out0_PORTS[
                                        (pid * 41) +
                                        9], Count_out0_PORTS[(pid * 41) + 10]

            for pid in range(0, 41):
                print "DPID : ", pid, "Count out 0 porta 52", Count_out0_PORT52[
                    pid]
            for pid in range(0, 41):
                print "DPID : ", pid, "Count in 0 porta 52", Count_in0_PORT52[
                    pid]
            for pid in range(0, 41):
                print "DPID : ", pid, "Count out 1 porta 52", Count_out1_PORT52[
                    pid]
            for pid in range(0, 41):
                print "DPID : ", pid, "Count in 1 porta 52 ", Count_in1_PORT52[
                    pid]
...
def launch(topo,
    ...
    if (IPdst != 0 and not is_valid_ipv4_address(IPdst)):
        raise RuntimeError("Wrong IPv4 address: " + str(IPdst))
    if (MACsrc != 0 and not is_valid_mac_address(MACsrc)):
        raise RuntimeError("Wrong MAC address: " + str(MACsrc))
    if (MACdst != 0 and not is_valid_mac_address(MACdst)):
        raise RuntimeError("Wrong MAC address: " + str(MACdst))

    if IPsrc != 0:
        print "Monitoring IP_SRC:", IPsrc
        ip_src = IPsrc
        var1 = 1

    if IPdst != 0:
        print "Monitoring IP_DST:", IPdst
        ip_dst = IPdst
        var2 = 1

    if MACsrc != 0:
        print "Monitoring MAC_SRC:", MACsrc
        mac_src = MACsrc
        var3 = 1

    if MACdst != 0:
        print "Monitoring MAC_DST:", MACdst
        mac_dst = MACdst
        var4 = 1

    if ip_src == 0 and ip_dst == 0 and mac_src == 0 and mac_dst == 0:
