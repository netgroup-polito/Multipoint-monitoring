...
def server():
    ...
    try:
        ...
        summary_lines.append(
            "Server listening on %s:%d" % (server_ip, SERVER_PORT))
        ...
        while not stop:
            ...
            if data['type'] == Type.SYN:
                ...
                summary_lines.append("%s disconnected, %d entries" %
                                     (data['source'], data['entries']))
                ...
                summary_lines.append("%s reported %d sent to %s" %
                                     (data['from'], data['sent'], data['to']))
                ...
                summary_lines.append("%s reported %d received from %s" %
                                     (data['to'], data['recv'], data['from']))

            if sorted(connected) == sorted(
                    disconnected) and num_entries == num_sent == num_recv:
                stop = True
        ...
        summary_lines.append(
            "Exception %s %s %s" % (e, exc_type, exc_tb.tb_lineno))
    finally:
